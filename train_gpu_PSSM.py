...
with open("train_index.pickle","rb") as fp:
    train_index = pickle.load(fp)
ids = list(map(lambda s: s.replace('.npy' , ''), train_index))

avgfp = pd.read_csv("./avgfp.tsv", sep = "\t")
avgfp = avgfp[avgfp["variant"].isin(ids)]
y = avgfp["score_wt_norm"].to_numpy()
y = y[:-2001]

#enc_avgfp = np.load("avgfp.npy")
#enc_seqs = np.expand_dims(enc_avgfp,1)
pssm_features = np.load("./pssm_features.npy")
pssm_features = np.expand_dims(pssm_features,1)
pssm_features = pssm_features[:-2001]

X_train, X_test, y_train, y_test = train_test_split(pssm_features,y,  test_size=0.2)
...
def build_model(model_type='regression', conv_layer_sizes=(16, 16, 16), dense_layer_size=16, dropout_rate=0.25):
    ...
    model.add(layers.ZeroPadding2D((1,1), input_shape = (1,20, 20)))
...
model_path =  './pssm_checkpoints.h5'
...
model.save("pssm_model.h5")

with open('./trainHistory_pssm.pickle', 'wb') as fp:
